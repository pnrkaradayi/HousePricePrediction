...
veriler_read = pd.read_excel('konutozlem2.xlsx', engine='openpyxl')
...
veriler_train = veriler_train[veriler_train['logm2'].notnull()]
veriler_train =veriler_train.reset_index(drop=True)
print (len(veriler_test))
print(len(veriler_train))
df_logprice =veriler_train.iloc[:,7:8]
df_features = veriler_train.iloc[:,0:7]
features = veriler_train.iloc[:,0:7].values
logprice =veriler_train.iloc[:,7:8].values
x_train1=df_features
y_train1=df_logprice
train11 = pd.concat([x_train1, y_train1], axis=1)
df_logprice_test =veriler_test.iloc[:,7:8]
df_features_test = veriler_test.iloc[:,0:8]
features_test = veriler_test.iloc[:,0:8].values
logprice_test =veriler_test.iloc[:,7:8].values
...
df_logprice_s =veriler_train2.iloc[:,9:10]
df_features_s = veriler_train2.iloc[:,0:8]
features_s = veriler_train2.iloc[:,0:8].values
logprice_s =veriler_train2.iloc[:,9:10].values
...
for i in range(0, len(cluster_list)):
    ...
    for train1, test1 in kf1.split(X1):
        ...
        # from sklearn.svm import NuSVR
        #
        # nusvr1 = NuSVR(nu=NU1, C=C1, kernel='rbf', gamma=G1)
        ...
        # print(fold1, fold2, "cluster : ", i, "C: ", C1, "Gamma :", G1, 'NU :', NU1, "mse :", nusvr1_mse1, "rmse :",
        #       nusvr1_rmse1, "mae :", nusvr1_mae1, "mape :", nusvr1_mape1, "r2 :", r2, "adjusted_r2score :",
        #       round(nusvr1.score(x_test1, y_test1), 2))
        # df_mse1 = df_mse1.append(pd.Series(
        #     [fold1, fold2, i, C1, G1, NU1, nusvr1_mse1, nusvr1_rmse1, nusvr1_mae1, nusvr1_mape1, r2,
        #      round(nusvr1.score(x_test1, y_test1), 2)],
        #     index=['fold1', 'fold2', 'i', 'c', 'gamma', 'nu', 'mse', 'rmse', 'mae', 'mape', 'r2',
